...
def get_all_users(exclude_id: int = None) -> list:
    ...
    if exclude_id:
        ...
        rows = conn.execute("SELECT id, display_name, role FROM users WHERE is_active = 1 ORDER BY display_name").fetchall()
    conn.close()
...
def create_or_get_dm(user1_id: int, user2_id: int) -> dict:
    ...
    u1 = conn.execute("SELECT display_name FROM users WHERE id = ?", (user1_id,)).fetchone()
    u2 = conn.execute("SELECT display_name FROM users WHERE id = ?", (user2_id,)).fetchone()
    if not u1 or not u2:
        conn.close()
        return {"error": "Пользователь не найден"}

    name = f"{u1['display_name']} ↔ {u2['display_name']}"
    cursor = conn.execute("INSERT INTO chats (name, chat_type) VALUES (?, 'dm')", (name,))
    chat_id = cursor.lastrowid
    conn.execute("INSERT INTO chat_members (chat_id, user_id) VALUES (?, ?)", (chat_id, user1_id))
    conn.execute("INSERT INTO chat_members (chat_id, user_id) VALUES (?, ?)", (chat_id, user2_id))
    conn.commit()
    conn.close()
...
def get_user_chats(user_id: int) -> list:
    ...
    for chat in result:
        ...
        if last:
            ...
            if last["message_type"] == "image": preview = "📷 Фото"
            elif last["message_type"] == "video": preview = "🎬 Видео"
            elif last["message_type"] == "file": preview = "📎 " + last["content"]
...
def save_message(chat_id: int, user_id: int, content: str, message_type: str = "text", file_path: str = None) -> dict:
    conn = get_db()
    cursor = conn.execute(
